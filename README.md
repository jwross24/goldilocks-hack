...
This is an archived hackathon submission. The `hackathon-submission` tag is the point-in-time record of what was submitted on May 26 2026; commits on `main` after that are post-event polish.

Issues are welcome — about the architecture, the multi-model loop, anything broken — but the project is not actively maintained. Fixes are best-effort.
